...
mutable struct BPETokenizer
    ...
    function BPETokenizer(vocab::Dict{String, Int}, merges::Vector{Tuple{String, String}},
            special_tokens::Dict{String, Int},
            special_token_properties::Dict{String, NamedTuple},
            cache::Dict{String, Vector{String}})
...
function load_tokenizer(config_path::String)
    ...
    return BPETokenizer(vocab, merges, special_tokens, special_token_properties,
...
function tokenize(tokenizer::BPETokenizer, text::AbstractString; token_ids::Bool = false)
    ...
    if token_ids
        return [get(tokenizer.special_tokens, t,
                    get(tokenizer.vocab, t, tokenizer.special_tokens["[UNK]"]))
                for t in tokens]
    end
...
function bpe_encode(tokenizer::BPETokenizer, word::AbstractString,
    ...
    if haskey(tokenizer.cache, cache_key)
        ...
        if token_ids
            ...
            return [get(tokenizer.vocab, t, tokenizer.special_tokens["[UNK]"])
    ...
    while iteration_count < max_iterations
        ...
        if !isempty(valid_merges)
            ...
            while i <= length(result)
                if i < length(result) && result[i] == best_pair[1] &&
                   result[i + 1] == best_pair[2]
    ...
    if !isempty(result) &&
       all(t -> haskey(tokenizer.vocab, t) || haskey(tokenizer.special_tokens, t), result)
    ...
    if token_ids
        ...
        for token in result
            if haskey(tokenizer.special_tokens, token)
                ...
                push!(token_ids,
                    get(tokenizer.vocab, token, tokenizer.special_tokens["[UNK]"]))
            end
        end
        return token_ids
    end
    ...
    while current_pos <= text_length
        ...
        for special_token in special_token_list
            if startswith(@view(text[current_pos:text_length]), special_token)
                ...
                # Advance position by the number of characters in special token
        ...
        while current_pos <= text_length
            if isspace(text[current_pos]) ||
               any(special -> startswith(@view(text[current_pos:text_length]), special),
                special_token_list)
                break
            end
            current_pos = nextind(text, current_pos)
        ...
        if current_pos > word_start
            word = text[word_start:prevind(text, current_pos)]
            # Special handling for sentence-initial capitalized words
            is_sentence_initial = tokenizer.prev_token == "[CLS]" && length(word) > 0 &&
                                  isuppercase(word[1])
            ...
            if is_sentence_initial && haskey(tokenizer.vocab, word)
                ...
                if !all(ispunct, word)  # Don't prefix punctuation
                    ...
                    is_after_special = haskey(
                        tokenizer.special_tokens, tokenizer.prev_token)
                    ...
                    if has_prefixed || has_unprefixed
                        ...
                        if (is_after_special || tokenizer.prev_token == "") && has_prefixed
                            word_tokens = token_ids ? [tokenizer.vocab[prefixed_word]] :
                                          [prefixed_word]
                        ...
                        prefixed_word = "Ġ" * word
                        if haskey(tokenizer.vocab, prefixed_word) &&
                           (is_after_special || tokenizer.prev_token == "")
                            word_tokens = token_ids ? [tokenizer.vocab[prefixed_word]] :
                                          [prefixed_word]
                        elseif haskey(tokenizer.vocab, word)
                            ...
                            word_tokens = bpe_encode(tokenizer, word,
                                is_after_special || tokenizer.prev_token == "", token_ids)
                        end
                    end
                else

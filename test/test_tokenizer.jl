using ModernBert: ModernBertTokenizer, encode, tokenize
...
@testset "Vocabulary and Special Tokens" begin
    ...
    @test vocab_size > 50288  # Expected vocabulary size + GPT2
    ...
    for (token, expected_id) in special_token_ids
        ...
        @test length(result) == 3  # Should be just the special token
        @test result[1][2] == expected_id
    end
end
...
@testset "Challenging Cases" begin
    ...
    for j in axes(tokens[1], 2)
        for i in axes(tokens[1], 1)
            ...
            check = tokens[1][i, j] == expected_ids[i, j]
            check ||
                @error "Test failed: Mismatch at token $i ($j: \"$(text)\"): got $(get(tokenizer.id_to_token, tokens[1][i,j], tokens[1][i,j])), expected $(get(tokenizer.id_to_token, expected_ids[i,j], expected_ids[i,j])), in token IDS: $(tokens[1][i, j]) vs $(expected_ids[i, j])"
            @test tokens[1][i, j] == expected_ids[i, j]
        end
    end
end

# tokenize(tokenizer, "O'Neill-McPherson's"; token_ids = false)
